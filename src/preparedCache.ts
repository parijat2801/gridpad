...
import { proseFontMeasure } from "./textFont";
...
export function buildPreparedCache(text: string): PreparedCache {
  ...
  return lines.map(line =>
    line.length > 0 ? prepareWithSegments(line, proseFontMeasure(), PREPARE_OPTS) : null
  );
}
...
export function invalidateLine(
  ...
  cache[lineNum] = newLineText.length > 0
    ? prepareWithSegments(newLineText, proseFontMeasure(), PREPARE_OPTS)
    : null;
}
...
export function splitLine(
  ...
  cache[row] = firstLineText.length > 0
    ? prepareWithSegments(firstLineText, proseFontMeasure(), PREPARE_OPTS)
    : null;
  cache.splice(row + 1, 0,
    secondLineText.length > 0
      ? prepareWithSegments(secondLineText, proseFontMeasure(), PREPARE_OPTS)
      : null
  );
}
...
export function mergeLines(
  ...
  cache[row - 1] = mergedLineText.length > 0
    ? prepareWithSegments(mergedLineText, proseFontMeasure(), PREPARE_OPTS)
    : null;
}
