...
// Re-export ProposedLayer here so scanner consumers don't have to reach
// into diff.ts for a type that represents the scanner's output.
export type { ProposedLayer } from "./diff";
import type { ProposedLayer } from "./diff";

/**
 * Convert a ScanResult into the ProposedLayer[] shape consumed by the
 * diff pass. Emission order matches the legacy buildLayersFromScan:
 * base (if unclaimed), rects, lines, texts. The diff pass uses this
 * order for z-assignment of new proposals.
 */
export function proposalsFromScan(result: ScanResult): ProposedLayer[] {
  const proposals: ProposedLayer[] = [];

  // Base layer — only when there are unclaimed cells
  if (result.unclaimedCells.size > 0) {
    let minR = Infinity,
      maxR = -Infinity,
      minC = Infinity,
      maxC = -Infinity;
    for (const k of result.unclaimedCells.keys()) {
      const i = k.indexOf(",");
    ...
    proposals.push({
      ...
      cells: new Map(result.unclaimedCells),
    });
  }

  // Rect layers — cells regenerated from extracted style
  for (const rect of result.rects) {
    const style = extractRectStyle(result.grid, rect);
    const bbox = { row: rect.row, col: rect.col, w: rect.w, h: rect.h };
    // Inline the rect perimeter painting to avoid a circular dep on layers.ts.
    // Matches regenerateCells(bbox, style).
    const cells = new Map<string, string>();
    const { row, col, w, h } = bbox;
    if (w === 1 && h === 1) {
      cells.set(`${row},${col}`, style.tl);
    } else if (h === 1) {
      cells.set(`${row},${col}`, style.tl);
      cells.set(`${row},${col + w - 1}`, style.tr);
      for (let c = col + 1; c < col + w - 1; c++) cells.set(`${row},${c}`, style.h);
    } else if (w === 1) {
      cells.set(`${row},${col}`, style.tl);
      cells.set(`${row + h - 1},${col}`, style.bl);
      for (let r = row + 1; r < row + h - 1; r++) cells.set(`${r},${col}`, style.v);
    } else {
      cells.set(`${row},${col}`, style.tl);
      cells.set(`${row},${col + w - 1}`, style.tr);
      cells.set(`${row + h - 1},${col}`, style.bl);
      cells.set(`${row + h - 1},${col + w - 1}`, style.br);
      for (let c = col + 1; c < col + w - 1; c++) {
        cells.set(`${row},${c}`, style.h);
        cells.set(`${row + h - 1},${c}`, style.h);
      }
      for (let r = row + 1; r < row + h - 1; r++) {
        cells.set(`${r},${col}`, style.v);
        cells.set(`${r},${col + w - 1}`, style.v);
      }
    }
    proposals.push({ type: "rect", bbox, cells, style });
  }

  // Line layers — cells copied verbatim
  for (const line of result.lines) {
    ...
    for (let r = minR; r <= maxR; r++) {
      for (let c = minC; c <= maxC; c++) {
        cells.set(`${r},${c}`, getCell(result.grid, r, c));
      }
    }
    proposals.push({
      type: "line",
  ...
  for (const text of result.texts) {
    const len = [...text.content].length;
    const cells = new Map<string, string>();
    for (let i = 0; i < len; i++) {
      cells.set(`${text.row},${text.col + i}`, getCell(result.grid, text.row, text.col + i));
    }
    proposals.push({
      type: "text",
      bbox: { row: text.row, col: text.col, w: len, h: 1 },
      cells,
